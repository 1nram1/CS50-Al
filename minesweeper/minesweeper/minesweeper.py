...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        #     cells = []
        #     if self.check_certainty((i - 1,j)):
        #         #if 
        #         cells.append((i - 1,j))
        #     if self.check_certainty((i - 1,j - 1)):
        #         cells.append((i - 1,j - 1))
        #     if self.check_certainty((i ,j - 1)):
        #         cells.append((i,j - 1))
        #     self.knowledge.append(Sentence(cells,count)  
        # elif i == self.height and j == 0
        neighbor = set()
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
        ...
        neighbor = neighbor - self.safes
        number_mines = len(neighbor & self.mines)
        neighbor = neighbor - self.mines
        self.knowledge.append(Sentence(neighbor,count - number_mines))

        # 第二阶段
        # 根据现有 knowledge, 不断地 conclude, 不断地 infer, 直到不能得到更多信息为止
        while True:
    ...
    def make_random_move(self):
        ...
        possible_move -= self.moves_made
        possible_move -= self.mines
        if len(possible_move) == 0 :
            return None
        return list(possible_move)[0]
